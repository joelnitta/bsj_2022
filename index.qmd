...
- どんな自動的なパイプラインでも、いくつかの**前提**や<br>**単純化**がある
...
本研究の目的：**シダ植物**を用いて、**分類学的に高精度**で**種数の多い**系統樹を自動的に作るシステムを開発
...
入力データは全部GenBankからダウンロードした葉緑体DNAシーケンスです。その中に、２種類のデータがあります。一つはサンガー法遺伝子です。本研究にはシダ植物で最もよく使われてきた７つの遺伝子を使っています。これは現在全部でやく５０００種が含まれています。
...
- IQ-TREE [@Nguyen2015] を用いて**葉緑体の系統樹を制約にして**最尤法によって系統樹を推定する
...
::: {.notes}
最後に、これからの予定と目的を説明させていただきたいと思います。
まずは、FTOLを完成させることです。現在のFTOLはシダ植物の半分弱を含めますが、残りの５０何パーセントを今後加えたいと考えています。
こうするためには、ずっとGenBankにデータが上がるのを待っているだけでは時間がかかり過ぎますので、今後は自分で新しいシーケンスデータを作る必要があります。特に有効だと思っているのは、植物標本庫に収まっている標本からゲノムスキミングによって葉緑体DNAシーケンスを読む技術です。
